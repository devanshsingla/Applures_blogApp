...
const createBlog = async (params) => {
    ...
    let query = 'Insert into blog(title,description,status,userID) VALUES(?,?,?,?)';
    console.log(params)

   return newPromiseDB(query,[params.title, params.description, params.status, params.userID])
}
...
const comments = async(params) => {
    ...
    if (!params.userID || !params.blogID || !params.comment) {
        return "Error in getting data "
    }
    
    const query = "Insert into comments(userId , blogID ,comment ) VALUES(?,?,?,?);"

    return newPromiseDB(query,[params.userID, params.blogID, params.comments])
};
...
const updateLike = async (params) => {
    ...
    return await newPromiseDB(query, [params.status , params.blogID, params.userID]);
...
const blogModels = {createBlog,likes,comments,getlike,updateLike,getAllLikes ,getAllBLog};
